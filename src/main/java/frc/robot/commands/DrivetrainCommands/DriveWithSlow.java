...
public class DriveWithSlow extends CommandBase {    
    ...
    public DriveWithSlow(drivetrain m_Drivetrain, DoubleSupplier translationSup, DoubleSupplier strafeSup, DoubleSupplier rotationSup, BooleanSupplier robotCentricSup, XboxController pov) {
    ...
    public void execute() {
        ...
        if(pov.getPOV() == 90){
            m_Drivetrain.drive(
                new Translation2d(0.2, .2).times(Constants.Swerve.maxSpeed), 
                rotationVal * Constants.Swerve.maxAngularVelocity, 
                !robotCentricSup.getAsBoolean(), 
                true
                );
